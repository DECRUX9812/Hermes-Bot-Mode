...
function trackInboundActivity(roster) {
  ...
  for (const bot of roster) {
    ...
    if ($activityToasts.get()) {
      ...
      // Cooldown: badge updates every poll, but toast at most once per bot
      // per TOAST_COOLDOWN_MS. Prevents squad chatter from pinging the human
      // every few seconds.
      const now = Date.now()
      const last = lastToastAt.get(bot.name) || 0
      if (now - last < TOAST_COOLDOWN_MS) {
        continue
      }
      lastToastAt.set(bot.name, now)
 ...
 *  uses the same dispatch. `fleet-dispatch` (the enforcement wrapper) is
 *  preferred; the raw chat command is the documented fallback. Recipient
 *  and sender are shell-quoted and the payload is JSON-quoted, so free-text
 *  titles and gateway profile names stay literal (fix #21 class). */
function fleetDispatchCommand(from, to, message) {
  const payload = JSON.stringify(`Message from 🤖 ${from} (@${from}): ${message}`)
  // Every interpolated value stays shell-literal (same class as #21): profile
  // names come from the gateway and titles sync from ui_meta — free text. The
  // fleet-dispatch script takes argv directly (no shell), so quoting here is
  // invisible to it; it only stops $() / backticks / quotes from expanding in
  // the agent's terminal before the script ever runs.
  return `fleet-dispatch send ${shellQuote(to)} ${shellQuote(payload)} --as ${shellQuote(from)}`
}

/** Raw chat fallback for hosts without the fleet-dispatch wrapper. Keeps
 *  every interpolated value shell-literal (same class as the routine-prompt
 *  fix #21): profile names and sender titles are free text from ui_meta. */
function rawChatCommand(from, to, message) {
  return `hermes -p ${shellQuote(to)} chat --in ~ -c "Bot Chat" -Q -q "Message from 🤖 ${shellDoubleQuote(from)} (@${shellDoubleQuote(from)}): ${message}"`
}
          ...
          jsxs('div', {
            ...
            children: [
              ...
              openLoops > 0
                ? jsx('span', {
                    className:
                      'shrink-0 rounded-full bg-(--chrome-action-hover) px-1.5 py-px text-[0.625rem] font-medium leading-4 text-amber-400',
...
function NeedsYou({ roster, unread, meta }) {
  ...
  return jsxs('div', {
    ...
    children: [
      jsxs('div', {
        className: 'flex items-center gap-1.5 px-2 py-1.5 text-[0.625rem] font-semibold uppercase tracking-wider text-amber-400',
        children: [
          jsx(Codicon, { name: 'alert', className: 'text-[0.8rem]' }),
          'Needs you',
          jsx('span', {
            className:
  ...
  return jsxs('div', {
    ...
    children: [
      ...
      roster.length
        ...
        : null,
      filteredRoster.length || summary.needYou || summary.working || summary.unread || summary.active || summary.paused
        ? jsxs('div', {
            className:
            ...
            children: [
              ...
              roster.length
                ? jsx('button', {
                    type: 'button',
                    className:
                      'ml-auto flex cursor-pointer items-center gap-1 rounded-full px-1.5 py-px text-[0.625rem] font-medium transition-colors hover:opacity-80 ' +
                      (allPaused ? 'text-amber-400' : 'text-(--ui-text-quaternary)'),
                    title: allPaused
                      ? 'Resume every bot — handoffs flow again'
                      : 'Pause every bot — blocks all handoff dispatch (in-app and via fleet-dispatch)',
                    onClick: () => {
                      for (const bot of roster) {
                      ...
                      host.notify({
                        kind: 'info',
                        message: allPaused
                          ? 'All bots resumed — handoffs flow again'
                          : 'All bots paused — handoffs blocked until you resume'
                      })
                    },
                    children: [allPaused ? '▶' : '⏸', allPaused ? 'resume all' : 'pause all']
                  })
                : null
            ]
          })
                  ...
                  children: jsx('div', {
                    ...
                    children: groupRoster(filteredRoster, allMeta).flatMap(section => [
                      ...
                      ...section.bots.map(bot =>
                        jsx(BotRow, { bot, onDelete: setDeleting, onEdit: setEditing, onGroup: setGrouping, openLoops: openLoops[bot.name] || 0 }, bot.name)
                      )
                    ])
                  })
                }),
