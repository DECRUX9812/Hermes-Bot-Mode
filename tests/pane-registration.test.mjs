...
test('registration: Fleet pane docks like Routines and renders FleetPage', () => {
  const entries = registeredItems()
  const fleet = entries.find(item => item.id === 'fleet')
  assert.ok(fleet, 'expected a fleet registration')
  assert.equal(fleet.title, 'Fleet')
  assert.equal(fleet.data.placement, 'main')
  // vm-realm objects: compare fields explicitly, never deepEqual.
  assert.equal(fleet.data.dock?.pane, 'workspace')
  assert.equal(fleet.data.dock?.pos, 'right')
  assert.equal(typeof fleet.render, 'function')
})
