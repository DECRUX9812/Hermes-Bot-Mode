...
const ROSTER = [
  { name: 'trader', role: 'Trader — markets & signals' },
  { name: 'researcher', role: 'Researcher — research, video, guides' },
  { name: 'scribe', role: 'Scribe — documentation & minutes' },
  { name: 'ops', role: 'Ops — operations & builds' },
  { name: 'teknium', role: 'Tech lead — infrastructure & fixes' },
  { name: 'manager', role: 'Manager — orchestration' },
...
function composeReport(profilesData, opts) {
  ...
  push(`- **Handoffs (${opts.windowHours}h):** ${handoffs.length} total — ${replied} replied, ${loopsTotal} open`)
  push(`- **Open loops:** ${loopsTotal}${openLoops.total ? ` (per bot: ${Object.entries(openLoops).filter(([k]) => k !== 'total').map(([k, v]) => `@${handle(byName.get(k) || { name: k })}×${v}`).join(', ')})` : ''}`)
